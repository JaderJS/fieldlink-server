...
const getProducts = async (req: FastifyRequest, res: FastifyReply) => {
    ...
    const categoriesQuery = await db.productCategory.findMany({
        where: whereCondition,
        include: {
            products: {
                include: {
...
const getProduct = async (req: FastifyRequest, res: FastifyReply) => {
    ...
    // const products = await db.product.findUniqueOrThrow({ where: { id }, include: { category: { include: { products: {} } } } })
    return res.send()
}
...
const updateProduct = async (req: FastifyRequest, res: FastifyReply) => {
    ...
    if (isDelete) {
        const drop = await db.product.delete({ where: { id } })
...
const upsertPurchase = async (req: FastifyRequest, res: FastifyReply) => {
    ...
    const cartMutation = await db.cart.upsert({
        where: { id },
        create: {
            status: cart.status,
    ...
    await db.productsOnCart.deleteMany({ where: { cartId: cart.id, productId: { notIn: cart.productsOnCart.map(({ id }) => id) } } })
    cart?.productsOnCart.map(async ({ price, quantity, ...product }) => {
        await db.productsOnCart.upsert({
    ...
    await db.otherValues.deleteMany({ where: { cart: { every: { id: id } }, id: { notIn: otherValues?.map(({ id }) => id) } } })
    ...
    await db.transactions.deleteMany({ where: { cartId: cart.id, id: { notIn: payment.transactions.map(({ id }) => id) } } })
    for (const [index, { id, periodId, value, fromAt, bankId }] of payment.transactions.entries()) {
        ...
        const transaction = await db.transactions.upsert({
            where: { id },
            create: {
                periodId,
                title,
