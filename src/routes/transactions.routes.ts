...
import { getTransaction, getTransactions, upsertTransaction, upsertTransactionPeriod } from "@/controllers/transaction.controller"
...
const transactions = async (server: FastifyInstance) => {
    ...
    server.post('/', { onRequest: [server.auth, server.authorize(['ROOT', 'ADMIN']), server.google] }, upsertTransaction)
